...
class Unknown(unittest.TestCase):
	...
	def test_a_placeholder_nothing_is_known_about_is_unknown(self):
		...
		(row,) = tree.flatten(entries, known={}, walking=True)
		self.assertEqual(row.status, tree.UNKNOWN)

	def test_a_downloaded_page_not_yet_read_is_still_walking(self):
		(row,) = tree.flatten(listing("caps.txt"), known={}, walking=True)
		self.assertEqual(row.status, tree.WALKING)

	def test_a_page_the_walk_finished_without_reading_is_unknown(self):
		# It would not read. The walk is over, so it is not still walking.
		(row,) = tree.flatten(listing("caps.txt"), known={}, walking=False)
		self.assertEqual(row.status, tree.UNKNOWN)
